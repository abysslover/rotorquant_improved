...
def verify_correctness():
    ...
    c_bivector = None
    c_trivector = getattr(rq, "centroids_trivector", None)
    if c_trivector is None:
        c_trivector = c_vector
...
def benchmark_rotor_sandwich():
    ...
    print(
        f"  {'n_vecs':>8s}  {'PyTorch (ms)':>14s}  {'Triton (ms)':>14s}  {'Speedup':>10s}"
    )
    print(f"  {'─' * 8}  {'─' * 14}  {'─' * 14}  {'─' * 10}")
...
def benchmark_fused_attention():
    ...
    print(
        f"  {'kv_len':>8s}  {'Standard (ms)':>14s}  {'Fused (ms)':>14s}  {'Speedup':>10s}  {'Cos sim':>10s}"
    )
    ...
    for kv_len in [128, 512, 1024, 2048, 4096]:
        ...
        k = torch.randn(
            batch, n_kv_heads, kv_len, d, device=device, dtype=torch.float32
        )
        ...
        def fused_fn():
            return triton_fused_attention(
                q_rot, k_indices, k_norms_reshaped, centroids, scale
            )
        ...
        print(
            f"  {kv_len:>8d}  {t_std:>14.3f}  {t_fused:>14.3f}  {speedup:>9.1f}x  {cos:>10.6f}"
        )
...
def benchmark_varying_dimensions():
    ...
    print(
        f"  {'dim':>6s}  {'n_groups':>8s}  {'PyTorch (ms)':>14s}  {'Triton (ms)':>14s}  {'Speedup':>10s}  {'Cos sim':>10s}"
    )
    ...
    if c_t is None:
        ...
        print(
            f"  {d:>6d}  {n_groups:>8d}  {t_pt:>14.3f}  {t_tr:>14.3f}  {speedup:>9.1f}x  {cos:>10.6f}"
        )
...
def benchmark_vs_turboquant():
    ...
    print(f"  d={d}\n")
    print(
        f"  {'bits':>4s}  {'n_vecs':>8s}  {'TQ-PyTorch':>14s}  {'RQ-PyTorch':>14s}  {'RQ-Triton':>14s}  {'TQ/Triton':>10s}"
    )
    ...
    if c_t is None:
        ...
        for n in [1024, 4096, 16384]:
            ...
            print(
                f"  {bits:>4d}  {n:>8d}  {t_tq:>12.3f}ms  {t_rq_pt:>12.3f}ms  "
...
def benchmark_bitwidth_sweep():
    ...
    print(f"  d={d}, n={n}\n")
    print(
        f"  {'bits':>4s}  {'n_levels':>8s}  {'MSE (PT)':>12s}  {'MSE (Tri)':>12s}  "
        f"{'PT (ms)':>10s}  {'Tri (ms)':>10s}  {'Speedup':>10s}"
    )
    print(
        f"  {'─' * 4}  {'─' * 8}  {'─' * 12}  {'─' * 12}  {'─' * 10}  {'─' * 10}  {'─' * 10}"
    )
    ...
    if c_t is None:
        ...
        print(
            f"  {bits:>4d}  {2**bits:>8d}  {mse_pt:>12.6f}  {mse_tr:>12.6f}  "
