...
def compute_perplexity(
    model, tokenizer, dataset_text, max_length=2048, stride=512, device="cuda"
):
...
def compute_perplexity_with_rq(
    model, tokenizer, dataset_text, bits=3, max_length=2048, stride=512, device="cuda"
):
...
def main():
    ...
    print(
        f"  {'Method':>15s}  {'PPL':>8s}  {'Delta':>8s}  {'%change':>8s}  {'Time':>8s}"
    )
    print(f"  {'─' * 15}  {'─' * 8}  {'─' * 8}  {'─' * 8}  {'─' * 8}")
    ...
    for bits in args.bits:
        ...
        print(
            f"  {'RQ ' + str(bits) + '-bit':>15s}  {ppl_rq:>8.2f}  {delta:>+8.2f}  {pct:>+7.1f}%  {t_rq:>6.1f}s"
        )
