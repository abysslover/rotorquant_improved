...
def make_patcher(bits, device="cuda"):
    ...
    def compress(ks, li):
        ...
        kq = triton_rotor_full_fused(flat, pk, None, c_vector, None, c_trivector)
...
def test_perplexity(model, tokenizer, bits_list, n_tokens=512, prefill_len=256):
    ...
    def _ar_eval(patch_fn=None, orig_fn=None):
        ...
        for i in range(input_ids.shape[1] - prefill_len):
            ...
            with torch.no_grad():
                out = model(
                    token, attention_mask=mask, past_key_values=cache, use_cache=True
                )
            cache = out.past_key_values
            logits = out.logits[:, -1:, :]
...
def test_niah(model, tokenizer, bits, contexts=[2048, 8192, 32768, 65536]):
    ...
    for ctx in contexts:
        ...
        inputs = tokenizer(
            prompt, return_tensors="pt", truncation=True, max_length=ctx + 500
        ).to("cuda")
        ...
        try:
            out = model.generate(
                **inputs, max_new_tokens=40, do_sample=False, use_cache=True
            )
...
def test_generation_quality(model, tokenizer, bits):
    ...
    for label, prompt in prompts:
        ...
        out = model.generate(
            **inputs, max_new_tokens=60, do_sample=False, use_cache=True
        )
...
def main():
    ...
    print(
        f"  Architecture: {n_layers} layers, {n_kv_heads} KV heads, head_dim={head_dim}, max_ctx={max_ctx:,}"
    )
    print()
    ...
    if not args.skip_niah:
        for bits in args.bits:
            ...
            for ctx, found, tok_s, vram, text in niah:
                ...
                print(
                    f"  {ctx:>8,}  {status:>8s}  {tok_s:>8.1f}/s  {vram:>6.0f}MB  {text}"
                )
            print()
    ...
    print(
        f"  RotorQuant:    4-bit at +{ppl_results[-1][3]:.1f}% PPL, NIAH ✅, 7x fewer FMAs"
    )
    print()
    print("=" * 70)
