"""
PlanarQuant High-Context POC

Patches Qwen2.5 models with PlanarQuant 2D Givens rotation KV cache compression
...
Usage:
    python -m methods.planarquant.planarquant_poc_high_context
    python -m methods.planarquant.planarquant_poc_high_context --bits 3 --max-ctx 65536

Based on: https://github.com/ParaMind2025/isoquant (PlanarQuant variant)
         https://arxiv.org/abs/2504.19874 (TurboQuant, ICLR 2026)

Note: PlanarQuant uses 2D Givens rotations for maximum speed with minimal quality loss.
"""
...
from methods.planarquant.planarquant import PlanarQuantMSE
...
# ── PlanarQuant Key Compressor ────────────────────────────────────────────


class PlanarQuantKeyCompressor:
    """Per-layer key compressor using PlanarQuant 2D Givens rotations."""
    ...
    def compress_dequantize(self, keys: torch.Tensor) -> torch.Tensor:
        """Quantize and dequantize keys using PlanarQuant."""
        B, H, S, D = keys.shape
        orig_dtype = keys.dtype
        flat = keys.reshape(-1, D)

        # Apply 2D Givens rotations (pairwise rotations)
        flat_rotated = flat @ self.rot2.T
        ...
        # Inverse rotation
        flat_recon = flat_recon @ self.rot2.T

        return flat_recon.to(orig_dtype).reshape(B, H, S, D)


class PlanarQuantValueCompressor:
    """Per-layer value compressor using PlanarQuant 2D Givens rotations."""
    ...
    def compress_dequantize(self, values: torch.Tensor) -> torch.Tensor:
        """Quantize and dequantize values using PlanarQuant."""
        B, H, S, D = values.shape
        orig_dtype = values.dtype
        flat = values.reshape(-1, D)

        # Apply 2D Givens rotations
        flat_rotated = flat @ self.rot2.T
...
# ── PlanarQuant-specific Patched Cache ─────────────────────────────────────


class PlanarQuantPatchedCache(PatchedCache):
    """PlanarQuant-specific implementation of PatchedCache."""

    def __init__(self, bits: int, device: str, quantize_values: bool = True):
        super().__init__(bits, device, quantize_values, backend="planarquant")
    ...
    def get_val_compressor(self, layer_idx: int, head_dim: int):
        if layer_idx not in self._val_compressors:
            self._val_compressors[layer_idx] = PlanarQuantValueCompressor(
                head_dim, self.bits, seed=layer_idx * 1000 + 500, device=self.device
            )
        return self._val_compressors[layer_idx]


def patch_planarquant_kv_cache(
    model,
    bits: int = 4,
    quantize_values: bool = False,
    device: str = "cuda",
):
    """Patch model for PlanarQuant KV cache compression.
    ...
    Returns:
        Tuple of (original_update, planarquant_cache)
    """
...
# ── PlanarQuant-specific Fidelity Measurement ──────────────────────────────


@torch.no_grad()
def measure_planarquant_fidelity(model, tokenizer, context_len: int, bits: int):
    """Measure PlanarQuant attention fidelity vs FP16.
    ...
    def get_compressor(layer_idx, head_dim):
        return PlanarQuantKeyCompressor(
            head_dim, bits, seed=layer_idx * 1000, device="cuda"
        )

    return measure_attention_fidelity(
        model, tokenizer, context_len, bits, "planarquant", get_compressor
    )


# ── PlanarQuant-specific Generation Test ───────────────────────────────────


@torch.no_grad()
def test_planarquant_generation(
    model,
    tokenizer,
    context_len: int,
    bits: int,
    ...
    """Generate text with PlanarQuant-compressed KV cache.
    ...
    def get_val_compressor(layer_idx, head_dim):
        return PlanarQuantValueCompressor(
...
def main():
    parser = argparse.ArgumentParser(description="PlanarQuant High-Context POC")
    ...
    print()
    print("=" * 74)
    print("  PlanarQuant High-Context POC")
    ...
    if not args.skip_fidelity:
        print("=" * 74)
        print("PHASE 1: Attention Fidelity (PlanarQuant vs FP16)")
        print("=" * 74)
        print()
        print(
        ...
        for ctx_len in ctx_lengths:
            if ctx_len > 8192:
                ...
                continue
            try:
                result = measure_planarquant_fidelity(
    ...
    print("=" * 74)
    print("PHASE 2: Generation with PlanarQuant (4-bit KV cache)")
    print("=" * 74)
    print()
    ...
    print("  PlanarQuant 4-bit results:")
    print()
    print(
    ...
    for ctx_len in ctx_lengths:
        ...
        try:
            result = test_planarquant_generation(
                model,
                tokenizer,
                ctx_len,
                args.bits,
    ...
    # PlanarQuant stores compressed indices, not full FP16
    pq_per_token = (n_layers * 2 * n_kv_heads * head_dim * args.bits) / 8  # compressed
    ...
    print(
        f"  PlanarQuant {args.bits}-bit per token: {pq_per_token:,} bytes ({pq_per_token / 1024:.1f} KB)"
    )
    print(f"  Compression: {fp16_per_token / pq_per_token:.1f}x")
    print(f"  GPU total: {gpu_total:.1f} GB, model: {model_vram / 1024:.1f} GB")
    print(f"  Available for KV cache: {avail_for_kv / 1024**3:.1f} GB")
    print()

    print(f"  {'Context':>10s}  {'FP16 KV':>10s}  {'PQ 4-bit':>10s}  {'Status'}")
    print(f"  {'─' * 10}  {'─' * 10}  {'─' * 10}  {'─' * 20}")
    for ctx in [2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288]:
        kv_bytes_fp16 = fp16_per_token * ctx
        kv_bytes_pq = pq_per_token * ctx
        kv_gb_fp16 = kv_bytes_fp16 / 1024**3
        kv_gb_pq = kv_bytes_pq / 1024**3
        fits = "fits" if kv_bytes_pq < avail_for_kv else "OOM"
        print(f"  {ctx:>10,}  {kv_gb_fp16:>8.2f}GB  {kv_gb_pq:>8.2f}GB  {fits}")
