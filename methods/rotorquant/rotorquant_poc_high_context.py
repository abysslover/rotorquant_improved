...
Patches Qwen2.5 models with RotorQuant Clifford algebra KV cache compression
...
Usage:
    python -m methods.rotorquant.rotorquant_poc_high_context
    python -m methods.rotorquant.rotorquant_poc_high_context --bits 3 --max-ctx 65536

Based on: https://arxiv.org/abs/2504.19874 (TurboQuant, ICLR 2026)

Note: RotorQuant uses Clifford algebra Cl(3,0) rotors for hardware-efficient rotation.
      (Legacy implementation - consider IsoQuant or PlanarQuant for better performance)
"""
...
from methods.rotorquant.rotorquant import RotorQuantMSE
...
# ── RotorQuant Key Compressor ────────────────────────────────────────────


class RotorQuantKeyCompressor:
    """Per-layer key compressor using RotorQuant Clifford rotors."""

    def __init__(self, head_dim: int, bits: int, seed: int, device: str):
        self.rq = RotorQuantMSE(head_dim, bits, seed=seed, device=device)
        self.rotors = self.rq.rotors.to(device)
        self.c_v = getattr(self.rq, "centroids_vector", None)
        if self.c_v is not None:
            self.c_v = self.c_v.to(device)
        self.c_t = getattr(self.rq, "centroids_trivector", None)
        if self.c_t is not None:
            self.c_t = self.c_t.to(device)
        self.head_dim = head_dim
        self.device = device

    @torch.no_grad()
    def compress_dequantize(self, keys: torch.Tensor) -> torch.Tensor:
        """Quantize and dequantize keys using RotorQuant."""
        B, H, S, D = keys.shape
        orig_dtype = keys.dtype
        flat = keys.reshape(-1, D)

        # Apply Clifford rotor sandwich: R @ v @ R~
        flat_rotated = flat @ self.rotors.T

        # Quantize and dequantize
        flat_quantized = (
            flat_rotated / self.rotors.abs().max() * 2 ** (bits - 1)
        ).long()
        ...
        # Inverse rotation
        flat_recon = flat_recon @ self.rotors.T

        return flat_recon.to(orig_dtype).reshape(B, H, S, D)


class RotorQuantValueCompressor:
    """Per-layer value compressor using RotorQuant Clifford rotors."""
    ...
    def compress_dequantize(self, values: torch.Tensor) -> torch.Tensor:
        """Quantize and dequantize values using RotorQuant."""
        B, H, S, D = values.shape
        orig_dtype = values.dtype
        flat = values.reshape(-1, D)

        # Apply Clifford rotor sandwich
        flat_rotated = flat @ self.rotors.T
...
# ── RotorQuant-specific Patched Cache ─────────────────────────────────────


class RotorQuantPatchedCache(PatchedCache):
    """RotorQuant-specific implementation of PatchedCache."""

    def __init__(self, bits: int, device: str, quantize_values: bool = True):
        super().__init__(bits, device, quantize_values, backend="rotorquant")
...
def patch_rotorquant_kv_cache(
    model,
    bits: int = 4,
    quantize_values: bool = False,
    device: str = "cuda",
):
    """Patch model for RotorQuant KV cache compression.
    ...
    Returns:
        Tuple of (original_update, rotorquant_cache)
    """
...
# ── RotorQuant-specific Fidelity Measurement ──────────────────────────────


@torch.no_grad()
def measure_rotorquant_fidelity(model, tokenizer, context_len: int, bits: int):
    """Measure RotorQuant attention fidelity vs FP16.
    ...
    def get_compressor(layer_idx, head_dim):
        return RotorQuantKeyCompressor(
            head_dim, bits, seed=layer_idx * 1000, device="cuda"
        )

    return measure_attention_fidelity(
        model, tokenizer, context_len, bits, "rotorquant", get_compressor
    )


# ── RotorQuant-specific Generation Test ───────────────────────────────────


@torch.no_grad()
def test_rotorquant_generation(
    model,
    tokenizer,
    context_len: int,
    bits: int,
    ...
    def get_val_compressor(layer_idx, head_dim):
        return RotorQuantValueCompressor(
...
def main():
    ...
    print()
    print("=" * 74)
    print("  RotorQuant High-Context POC (Legacy)")
    ...
    print("=" * 74)
    print("  Note: For better performance, consider IsoQuant or PlanarQuant")
    print()
    ...
    if not args.skip_fidelity:
        ...
        for ctx_len in ctx_lengths:
            if ctx_len > 8192:
                ...
                continue
            try:
                result = measure_rotorquant_fidelity(
                    model, tokenizer, ctx_len, args.bits
                )
                print(
    ...
    print("=" * 74)
    print("PHASE 2: Generation with RotorQuant (4-bit KV cache)")
    print("=" * 74)
    print()
    ...
    print("  RotorQuant 4-bit results:")
    print()
    print(
    ...
    for ctx_len in ctx_lengths:
        ...
        try:
            result = test_rotorquant_generation(
                model,
                tokenizer,
                ctx_len,
                args.bits,
    ...
    # RotorQuant stores compressed indices, not full FP16
    rq_per_token = (n_layers * 2 * n_kv_heads * head_dim * args.bits) / 8  # compressed
    ...
    print(
        f"  RotorQuant {args.bits}-bit per token: {rq_per_token:,} bytes ({rq_per_token / 1024:.1f} KB)"
    )
    print(f"  Compression: {fp16_per_token / rq_per_token:.1f}x")
    print(f"  GPU total: {gpu_total:.1f} GB, model: {model_vram / 1024:.1f} GB")
    print(f"  Available for KV cache: {avail_for_kv / 1024**3:.1f} GB")
    print()

    print(f"  {'Context':>10s}  {'FP16 KV':>10s}  {'RQ 4-bit':>10s}  {'Status'}")
    print(f"  {'─' * 10}  {'─' * 10}  {'─' * 10}  {'─' * 20}")
    for ctx in [2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288]:
        kv_bytes_fp16 = fp16_per_token * ctx
        kv_bytes_rq = rq_per_token * ctx
        kv_gb_fp16 = kv_bytes_fp16 / 1024**3
        kv_gb_rq = kv_bytes_rq / 1024**3
        fits = "fits" if kv_bytes_rq < avail_for_kv else "OOM"
        print(f"  {ctx:>10,}  {kv_gb_fp16:>8.2f}GB  {kv_gb_rq:>8.2f}GB  {fits}")
