"""
TurboQuant High-Context POC

Patches Qwen2.5 models with TurboQuant MSE-only KV cache compression
...
Usage:
    python -m methods.turboquant.turboquant_poc_high_context
    python -m methods.turboquant.turboquant_poc_high_context --bits 3 --max-ctx 65536

Based on: https://arxiv.org/abs/2504.19874 (TurboQuant, ICLR 2026)

Note: TurboQuant uses MSE-only for KV cache (QJL removed per empirical findings).
"""
...
class TurboQuantKeyCompressor:
    """Per-layer key compressor using TurboQuant MSE + PyTorch quantization."""

    def __init__(self, head_dim: int, bits: int, seed: int, device: str):
        self.tq = TurboQuantMSE(head_dim, bits, seed=seed, device=device)
        self.head_dim = head_dim
        self.device = device

    @torch.no_grad()
    def compress_dequantize(self, keys: torch.Tensor) -> torch.Tensor:
        """Quantize and dequantize keys using TurboQuant."""
        B, H, S, D = keys.shape
        orig_dtype = keys.dtype
        flat = keys.reshape(-1, D)
        flat_recon, _ = self.tq.quantize(flat)
        return flat_recon.to(orig_dtype).reshape(B, H, S, D)


class TurboQuantValueCompressor:
    """Per-layer value compressor using TurboQuant MSE."""
    ...
    def compress_dequantize(self, values: torch.Tensor) -> torch.Tensor:
        """Quantize and dequantize values using TurboQuant."""
...
# ── TurboQuant-specific Patched Cache ─────────────────────────────────────


class TurboQuantPatchedCache(PatchedCache):
    """TurboQuant-specific implementation of PatchedCache."""

    def __init__(self, bits: int, device: str, quantize_values: bool = True):
        super().__init__(bits, device, quantize_values, backend="turboquant")
    ...
    def get_key_compressor(self, layer_idx: int, head_dim: int):
        if layer_idx not in self._key_compressors:
            self._key_compressors[layer_idx] = TurboQuantKeyCompressor(
    ...
    def get_val_compressor(self, layer_idx: int, head_dim: int):
        if layer_idx not in self._val_compressors:
            self._val_compressors[layer_idx] = TurboQuantValueCompressor(
                head_dim, self.bits, seed=layer_idx * 1000 + 500, device=self.device
            )
        return self._val_compressors[layer_idx]


def patch_turboquant_kv_cache(
    model, bits: int = 4, quantize_values: bool = False, device: str = "cuda"
):
    """Patch model for TurboQuant KV cache compression.
    ...
    Returns:
        Tuple of (original_update, turboquant_cache)
    """
...
# ── TurboQuant-specific Fidelity Measurement ──────────────────────────────


@torch.no_grad()
def measure_turboquant_fidelity(model, tokenizer, context_len: int, bits: int):
    """Measure TurboQuant attention fidelity vs FP16.
    ...
    def get_compressor(layer_idx, head_dim):
        return TurboQuantKeyCompressor(
            head_dim, bits, seed=layer_idx * 1000, device="cuda"
        )

    return measure_attention_fidelity(
        model, tokenizer, context_len, bits, "turboquant", get_compressor
    )


# ── TurboQuant-specific Generation Test ───────────────────────────────────


@torch.no_grad()
def test_turboquant_generation(
    model,
    tokenizer,
    context_len: int,
    bits: int,
    ...
    def get_val_compressor(layer_idx, head_dim):
        return TurboQuantValueCompressor(
...
def main():
    parser = argparse.ArgumentParser(description="TurboQuant High-Context POC")
    ...
    print()
    print("=" * 74)
    print("  TurboQuant High-Context POC")
    ...
    if not args.skip_fidelity:
        print("=" * 74)
        print("PHASE 1: Attention Fidelity (TurboQuant vs FP16)")
        print("=" * 74)
        print()
        print(
        ...
        for ctx_len in ctx_lengths:
            if ctx_len > 8192:
                ...
                continue
            try:
                result = measure_turboquant_fidelity(
    ...
    print("=" * 74)
    print("PHASE 2: Generation with TurboQuant (4-bit KV cache)")
    print("=" * 74)
    print()
    ...
    print("  TurboQuant 4-bit results:")
    print()
    print(
    ...
    for ctx_len in ctx_lengths:
        ...
        try:
            result = test_turboquant_generation(
                model,
                tokenizer,
                ctx_len,
                args.bits,
    ...
    # TurboQuant stores compressed indices, not full FP16
    ...
    print(
        f"  TurboQuant {args.bits}-bit per token: {tq_per_token:,} bytes ({tq_per_token / 1024:.1f} KB)"
    )
    ...
    print(f"  {'Context':>10s}  {'FP16 KV':>10s}  {'TQ 4-bit':>10s}  {'Status'}")
    print(f"  {'─' * 10}  {'─' * 10}  {'─' * 10}  {'─' * 20}")
    for ctx in [2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288]:
        kv_bytes_fp16 = fp16_per_token * ctx
        kv_bytes_tq = tq_per_token * ctx
        kv_gb_fp16 = kv_bytes_fp16 / 1024**3
        kv_gb_tq = kv_bytes_tq / 1024**3
        fits = "fits" if kv_bytes_tq < avail_for_kv else "OOM"
        print(f"  {ctx:>10,}  {kv_gb_fp16:>8.2f}GB  {kv_gb_tq:>8.2f}GB  {fits}")
